...
public unsafe class cuteHeiBian2
{
    uint ic = 0;
    uint w = 0;
    uint h = 0;
        ...
        for (uint y = 0; y < h; y++)
        {
            for (uint x = 0; x < w; x++)
            {
                if (x == 0 || x == w - 1 || y == 0 || y == h - 1)
                {
                    ...
                    rgb = new dotRGB(p[ib], p[ib + 1], p[ib + 2]);

                    if (rgb.isGrey && arrRect_P[x, y] == 0)
                    {
                        //基点入锥
                        Al_sp = new ArrayList();
                        //arrRect_P[x, y] = 1;
                            ...
                            for (int irow = -1; irow <= 1; irow++)
                            {
                                for (int icell = -1; icell <= 1; icell++)
                                {
                                    ...
                                    if (thisx >= 0 && thisx < w && thisy >= 0 && thisy < h)
                                    {
                                        ...
                                        rgb2 = new dotRGB(p[ib2], p[ib2 + 1], p[ib2 + 2]);

                                        if (rgb2.isGrey && Math.Abs(rgb.iGrey - rgb2.iGrey) <= 5 && arrRect_P[thisx, thisy] == 0)
                                        {
                                            //新点入锥
    ...
    /// <returns></returns>
    public unsafe Bitmap setImage(FileInfo f, ref Bitmap bmp)
    {
        isRectFrame = false;
        isNeedDo = false;
        isBuBig = false;
        ...
        //联通区域 (特殊：区域必须贴边)
        ArrayList arrRectBu = cutArea(p, file);


        iColorBu = getAvgGrey(p, 80, w, h);
        //iColorBu = new dotRGB(255, 255, 255);
    ...
    /// <param name="ih"></param>
    /// <returns></returns>
    public unsafe cuteHeiBian.dotRGB getAvgGrey(byte* p, uint Ostu, uint iw, uint ih)
    {
        cuteHeiBian.greyRGB[] arrDot = new cuteHeiBian.greyRGB[256];
        for (int i = 0; i < arrDot.Length; i++)
        {
            arrDot[i] = new cuteHeiBian.greyRGB();
        }
    ...
    public class dotRGB
    {
        ...
        public dotRGB(byte r, byte g, byte b)
        {
            ...
            G = g;
            B = b;
            byte iGrey = (byte)(((int)R + (int)G + (int)B) / 3);
            ...
            if (R > 100 && R < 245)
            {
                ...
                if (Math.Abs(R - iGrey) < 5 && Math.Abs(G - iGrey) < 5 && Math.Abs(B - iGrey) < 5)
                {
                    isGrey = true;
                }
            }
