...
class imageSetting
{
    ...
    public imageSetting(ref Bitmap bmp)
    {
        bitmap = bmp;
        setDefault();
    }
    public imageSetting(FileInfo f, string save)
    {
        file = f;
    ...
    public imageSetting(ref Bitmap bmp, string save)
    {
        bitmap = bmp;
        savePath = save;
        setDefault();
    ...
    {
        bitmap.Save(spath, myImageCodecInfo, encoderParams);
    }
    public unsafe void InitNoDispose()
    {
        Init(false);
    }
    ...
    public unsafe void Init(bool isDispose)
    {
        //Graphics g = null;
        ...
        //
        toConstract(p, iBright, iConstract);
        //toBright(p, 30);
        //toConstract(p, 20, 0);
    ...
    /// </summary>
    /// <returns></returns>
    public unsafe void toConstract(byte* p, int degree, double iValue)
    {
        ...
        for (uint y = 0; y < h; y++)
        {
            for (uint x = 0; x < w; x++)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    pixel = ((p[ib + i] / 255.0 - 0.45) * contrast + 0.5) * 255;
                    if (pixel < 0) pixel = 0;
